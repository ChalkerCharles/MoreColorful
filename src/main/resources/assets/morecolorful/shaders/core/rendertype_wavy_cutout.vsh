...
void main() {
    ...
    switch (type) {
        case 1:
            xs = sin(position.x + position.y + animation * speed_multiplier(speedX)) * speedX / 8.0 + speedX / 5.0;
            zs = cos(position.z + position.y + animation * speed_multiplier(speedZ)) * speedZ / 8.0 + speedZ / 5.0;
            break;
        case 2:
            xs = sin(position.x + (position.y / 2.0) + animation * speed_multiplier(speedX)) * speedX / 5.0 + offset.x;
            ys = offset.y;
            zs = cos(position.z + (position.y / 2.0) + animation * speed_multiplier(speedZ)) * speedZ / 5.0 + offset.z;
            break;
        case 3:
            ...
            break;
        case 4:
            vec3 offset_lantern = accumulate_offset_chain(animation, vec3(speedX, speedY, speedZ), windSpeed, tip, height, Wave.w);
            xs = offset_lantern.x;
            ys = offset_lantern.y;
            zs = offset_lantern.z;
            break;
        case 5:
            xs = sin(position.x + position.y + animation * speed_multiplier(speedX)) * speedX / 5.0 + offset.x;
            ys = offset.y;
            zs = cos(position.z + position.y + animation * speed_multiplier(speedZ)) * speedZ / 5.0 + offset.z;
            break;
        default:
            xs = sin(position.x + position.y + animation * speed_multiplier(speedX)) * speedX / 5.0 + offset.x;
            ys = -offset.y;
